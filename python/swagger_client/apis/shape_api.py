...
class ShapeApi(object):
    ...
    def api_shape_controller_index(self, filter_route, **kwargs):
        """
        ...
        **NOTE:** `filter[route]` **MUST** be given for any shapes to be returned.  List of shapes.  ## Vertices  ### World  `/data/{index}/attributes/polyline` is in [Encoded Polyline Algorithm Format](https://developers.google.com/maps/documentation/utilities/polylinealgorithm), which encodes the latitude and longitude of a sequence of points in the shape.  ### Stops  If instead of getting the latitude and longitude directly, you want to show the stops in this shape use `/data/{index}/relationships/stops` to get the all the stop IDs or `include=stops` to include them in the response.  
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.api_shape_controller_index(filter_route, callback=callback_function)
        ...
        if kwargs.get('callback'):
            return self.api_shape_controller_index_with_http_info(filter_route, **kwargs)
        else:
            (data) = self.api_shape_controller_index_with_http_info(filter_route, **kwargs)
            return data

    def api_shape_controller_index_with_http_info(self, filter_route, **kwargs):
        """
        ...
        **NOTE:** `filter[route]` **MUST** be given for any shapes to be returned.  List of shapes.  ## Vertices  ### World  `/data/{index}/attributes/polyline` is in [Encoded Polyline Algorithm Format](https://developers.google.com/maps/documentation/utilities/polylinealgorithm), which encodes the latitude and longitude of a sequence of points in the shape.  ### Stops  If instead of getting the latitude and longitude directly, you want to show the stops in this shape use `/data/{index}/relationships/stops` to get the all the stop IDs or `include=stops` to include them in the response.  
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.api_shape_controller_index_with_http_info(filter_route, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method api_shape_controller_index" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'filter_route' is set
        if ('filter_route' not in params) or (params['filter_route'] is None):
            raise ValueError("Missing the required parameter `filter_route` when calling `api_shape_controller_index`")

        if 'page_offset' in params and params['page_offset'] < 0:
            raise ValueError("Invalid value for parameter `page_offset` when calling `api_shape_controller_index`, must be a value greater than or equal to `0`")
        if 'page_limit' in params and params['page_limit'] < 1:
            raise ValueError("Invalid value for parameter `page_limit` when calling `api_shape_controller_index`, must be a value greater than or equal to `1`")
    ...
    def api_shape_controller_show(self, id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.api_shape_controller_show(id, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.api_shape_controller_show_with_http_info(id, **kwargs)
        else:
            (data) = self.api_shape_controller_show_with_http_info(id, **kwargs)
            return data

    def api_shape_controller_show_with_http_info(self, id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.api_shape_controller_show_with_http_info(id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method api_shape_controller_show" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'id' is set
        if ('id' not in params) or (params['id'] is None):
            raise ValueError("Missing the required parameter `id` when calling `api_shape_controller_show`")
